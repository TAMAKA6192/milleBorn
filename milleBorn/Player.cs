...
public class Player {
    ...
    public bool CanPlayDistance(int distance) {
        ...
        var maxDistance = HasSpeedLimit ? 50 : int.MaxValue;
        return distance <= maxDistance && TotalDistance + distance <= 1000;
    }
    ...
    public bool CanPlayRemedy(Card remedy) => remedy.Category == CardCategory.Remedy && remedy.Type switch {
    ...
    public bool IsProtectedFrom(CardType hazardType) => hazardType switch {
    ...
    public string GetStatus() {
        var status = $"{Name}: {TotalDistance}km";
