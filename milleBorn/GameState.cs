namespace milleBorn
{
    public class GameState
    {
        ...
        public List<Player> GetValidTargets(Player attacker, Card hazardCard)
        {
            if (hazardCard.Category != CardCategory.Hazard)
        ...
        {
            string status = "\n=== ƒQ[ƒ€ó‹µ ===\n";
            status += $"ŽRŽD: {Deck.Count}–‡\n";
            status += $"ŽÌŽD: {DiscardPile.Count}–‡\n\n";
            ...
            foreach (var player in Players)
            {
                ...
                {
                    status += $"  ˆÀ‘SƒJ[ƒh: {string.Join(", ", player.SafetyCards.Select(c => c.Name))}\n";
                }
            }
        ...
        public bool IsGameOver()
        {
            ...
            if (Deck.Count == 0)
            {
                bool anyoneCanPlay = Players.Any(p => p.Hand.Any());
                if (!anyoneCanPlay)
                {
                    GameEnded = true;
