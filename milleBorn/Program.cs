...
internal class Program {
    private static void Main(string[] args) {
        ...
        {
            new("あなた", isHuman: true),
            new("CPU1", isHuman: false),
            new("CPU2", isHuman: false),
            new("CPU3", isHuman: false)
        };
    ...
    private static void PlayHumanTurn(Player player, GameState gameState) {
        ...
        while (!turnComplete) {
            ...
            for (var i = 0; i < player.Hand.Count; i++) {
                ...
                var info = GetCardInfo(card, player);
            ...
            if (int.TryParse(Console.ReadLine(), out var choice) &&
                ...
                if (selectedCard.Category == CardCategory.Hazard) {
                    ...
                    if (targets.Any()) {
                        ...
                        if (int.TryParse(Console.ReadLine(), out var targetChoice)) {
                            if (targetChoice == 0) {
                                continue;
                            }
                        ...
                        if (Console.ReadLine()?.ToLower() == "y") {
                            _ = gameState.DiscardCard(player, selectedCard);
    ...
    private static void PlayAITurn(Player player, GameState gameState, AIPlayer ai) {
        var (card, target) = ai.ChooseAction(player, gameState);
        var action = ai.ExplainAction(card, target);

        Console.WriteLine($"{player.Name}: {action}");

        if (target != null) {
            if (!gameState.PlayCard(player, card, target)) {
                _ = gameState.DiscardCard(player, card);
            }
        } else {
    ...
    private static string GetCardInfo(Card card, Player player) => card.Category switch {
